...
export const api = {
  ...
  getControlOverview: (refresh = false) =>
    fetchJSON<ControlOverviewResponse>(
      `/api/control/overview?refresh=${refresh ? "true" : "false"}`,
    ),
  getControlProjects: (domain = "all", refresh = false) =>
    fetchJSON<ControlProjectsResponse>(
      `/api/control/projects?domain=${encodeURIComponent(domain)}&refresh=${refresh ? "true" : "false"}`,
    ),
  getControlConnectors: () =>
    fetchJSON<ControlConnectorsResponse>("/api/control/connectors"),
  getControlSpend: () => fetchJSON<ControlSpendResponse>("/api/control/spend"),
  getControlActivity: () =>
    fetchJSON<ControlActivityResponse>("/api/control/activity"),
  getDispatchBoard: () =>
    fetchJSON<ControlDispatchBoardResponse>("/api/control/dispatch/board"),
  getControlZaiUsage: () =>
    fetchJSON<ControlZaiUsageResponse>("/api/control/zai_usage"),
  getControlVitals: () =>
    fetchJSON<ControlVitalsResponse>("/api/control/vitals"),
  getControlAutomations: () =>
    fetchJSON<ControlAutomationsResponse>("/api/control/automations"),
  getControlAttention: () =>
    fetchJSON<ControlAttentionResponse>("/api/control/attention"),
  getControlKnowledgeGraph: () =>
    fetchJSON<ControlKnowledgeGraphResponse>("/api/control/knowledge_graph"),
...
export interface ControlDetectedMarkers {
  readme?: boolean;
  claudeMd?: boolean;
  agentsMd?: boolean;
  packageJson?: boolean;
...
export interface ControlProjectRoot {
  id: string;
  label: string;
  domain: "krendora" | "ticketflipping" | "system";
  path: string;
  source: "pc_bridge" | "local";
  caution: "normal" | "high" | "production";
...
export interface ControlProject {
  ...
  source: "pc_bridge" | "local";
  path: string;
  kind: "repo" | "workspace" | "file" | "unknown";
  summary?: string;
  caution: "normal" | "high" | "production";
  restricted: boolean;
  detected: ControlDetectedMarkers;
  last_seen_at: string;
}

export interface ControlConnectorSummary {
  id: string;
  label: string;
  kind: string;
  status: "connected" | "configured" | "disabled" | "warning" | "error" | "unknown";
  detail?: string | null;
  updated_at?: string | null;
  latency_ms?: number | null;
  href?: string | null;
...
export interface ControlOverviewResponse {
  generated_at: string;
  bridge: PcBridgeStatus;
  roots: ControlProjectRoot[];
  projects: ControlProject[];
  connectors: ControlConnectorSummary[];
  attention: ControlAttentionItem[];
}

export interface ControlProjectsResponse {
  generated_at: string;
  bridge: PcBridgeStatus;
  roots: ControlProjectRoot[];
  projects: ControlProject[];
...
export interface ControlConnectorsResponse {
  generated_at: string;
  connectors: ControlConnectorSummary[];
}

export interface ControlSpendModelUsage {
  model: string;
  usd: number;
...
export interface ControlSpendResponse {
  generated_at: string;
  model: { name: string; provider: string; short: string };
  pricing: {
    input_per_m: number | null;
    cached_per_m: number | null;
  ...
  usage: {
    ...
    by_model: ControlSpendModelUsage[];
    available: boolean;
    error?: string | null;
  };
  expenses: {
...
export interface ControlActivitySession {
  id: string;
  title: string;
  source: string;
  model: string;
...
export interface ControlDelegation {
  id: string;
  kind: "voice" | "agent" | string;
  goal: string;
  state: string;
  ago_seconds: number | null;
  progress: number | null;   // 0..1, or null when no honest estimate is possible
  eta_seconds: number | null; // seconds remaining, or null → "No ETA"
  note?: string;             // agent's own status note (self-reported)
  self?: boolean;            // true when the agent self-reported this progress
}

export interface ControlActivityResponse {
  generated_at: string;
  sessions: ControlActivitySession[];
  active_count: number;
  delegations_inflight: number;
  delegations?: ControlDelegation[];
  available: boolean;
  error?: string | null;
}

export interface ControlZaiWindow {
  key: string;
  label: string;
  used: number | null;
  limit: number | null;
...
export interface ControlZaiUsageResponse {
  generated_at: string;
  available: boolean;
  level: string | null;
  windows: ControlZaiWindow[];
  error?: string | null;
}

export interface ControlVitalsResponse {
  cpu_pct: number | null;
  mem_used_mb?: number;
  mem_total_mb?: number;
  disk_used_gb?: number;
...
export interface ControlAutomationsResponse {
  available: boolean;
  items: ControlAutomation[];
  active: number;
  failing: number;
}

export interface ControlAttentionAlert {
  severity: string;
  title: string;
  msg: string;
  kind: string;
...
export interface ControlAttentionResponse {
  alerts: ControlAttentionAlert[];
  count: number;
  critical: number;
}

export interface ControlGraphRepo {
  name: string;
  domain: string;
  nodes: number;
  edges: number;
...
export interface ControlKnowledgeGraphResponse {
  available: boolean;
  repos: ControlGraphRepo[];
  totals?: { nodes: number; edges: number; repos: number };
}
...
export interface ControlDispatchThread {
  slug: string;
  title: string;
  project: string;
  status: string;
  open_count: number;
  items: ControlDispatchItem[];
}
export interface ControlDispatchBoardResponse {
  generated_at: string;
  counts: Record<string, number>;
  threads: ControlDispatchThread[];
  unclassified: ControlDispatchItem[];
  available: boolean;
  error?: string | null;
}
